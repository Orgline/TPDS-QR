...
void hou_caqr_panel(cublasHandle_t handle, long int m, long int n, float *A, long int lda, float *R, long int ldr, float *work)
{
    ...
    if ((m - m / M * M) % N != 0)
    {
        printf("Error: m must be i*%ld + j*%ld\n", M, N);
    }
    long int NB = (m + M - 1) / M;
...
void RHOUQR(cusolverDnHandle_t cusolver_handle, cublasHandle_t cublas_handle, long currentN, long mt, long nt, float* A, long lda, float* W, long ldw, float* R, long ldr, float* work, int* info)
{
...
int main(int argc, char *argv[])
{
    ...
    //(cusolverDnHandle_t cusolver_handle, cublasHandle_t cublas_handle, long currentN, long m, long n, float* A, long lda, float* W, long ldw, float* R, long ldr, float* work, int* info)
